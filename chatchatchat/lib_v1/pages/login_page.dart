...
import 'package:chatchatchat/plugin/error_message.dart';
...
class _LoginPageState extends State<LoginPage> {
  ...
  _signInWithGoogle() async {
    ...
    await _firebaseAuth.signInWithCredential(credential).then((result){
      ...
      temp.add(user.displayName.toLowerCase());
      temp.add(user.email.toLowerCase());
      if(user.phoneNumber != null){
        temp.add(user.phoneNumber.toString().toLowerCase());
      }
      for(int i = 0; i < temp.length; i++){
        String baseString = temp[i];
        for(int i = 0; i <= baseString.length; i++){
          String x = baseString.substring(0,i);
      ...
      _fireStore.collection('users').document(userId).get().then((value){
        _firebaseMessaging.getToken().then((tokenValue){
          if(value.data == null){
            _fireStore.collection('users').document(userId).setData({
              ...
              'friends' : [],
              'requests' : [],
            }, merge: true).then((x){ }).catchError((e){
              _signOut().then((x){
                showError(context, 'An error occoured. Please retry logging in.');
              });
            });
